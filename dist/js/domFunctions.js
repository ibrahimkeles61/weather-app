	...
	if (message.indexOf("Lat:") !== -1 && message.indexOf("Long:") !== -1) {
		...
		const lat =
			msgArray[1].indexOf("-") === -1
				? msgArray[1].slice(0, 5)
				: msgArray[1].slice(0, 6);
		const lon =
			msgArray[3].indexOf("-") === -1
				? msgArray[3].slice(0, 5)
				: msgArray[3].slice(0, 6);
...
const fadeDisplay = () => {
	...
	const sicday = document.getElementById("dailyForecast");
	sicday.classList.toggle("zero-vis");
	sicday.classList.toggle("fade-in");
};
